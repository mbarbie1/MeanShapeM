...
methods = { 'imagej' }%, 'meanshape' };
    ...
    for i = 1:n_masks
        ...
        s = regexp( file_name, '^(?<id>.*)_imagej_(?<mask_id>\d+).roi', 'names' );
        cell_id = 1;
    ...
    writetable( mask_table, fullfile( output_folder, 'mask_table.csv' ) );
...
% 
    ...
    for i = 1:n_masks
        ...
        sRoi = ReadImageJROI( mask_table{i,'file_path'} );
        ...
        if strcmp( sRoi(1).strType, 'Rectangle')
            continue;
            rr = sRoi(1).vfShapes;
            sRoi(1).mnCoordinates = [ rr(2:3:end), rr(3:3:end) ];
            disp('Rectangle');
        end
        p = sRoi(1).mnCoordinates';
